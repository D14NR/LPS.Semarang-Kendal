  ...
  return (
    <>
      <CrudPage
        ...
        modalSize="lg"
        addLabel="Tambah Nilai Tes Standar"
      ...
      <Modal isOpen={addOpen} onClose={() => setAddOpen(false)} title="Tambah Nilai Tes Standar" size="lg">
        <form
          onSubmit={async (e) => {
            e.preventDefault();
            if (!tanggal || !jenisTes || !mataPelajaran) return;
            ...
            setSubmitting(true);
            try {
              const res = await createBulkRecords('nilaiTesStandar', payloads);
              if (res.success) {
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Jenis Tes</label>
              <input value={jenisTes} onChange={(e) => setJenisTes(e.target.value)} className="w-full px-3 py-2 border rounded-lg" />
            </div>
            <div>
          ...
          {tanggal && jenisTes && mataPelajaran && (
            <div className="mb-3">
