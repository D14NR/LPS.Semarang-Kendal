...
import { fetchAllData, fetchPengajarFromKmb, isAppsScriptConfigured, getSpreadsheetUrl, invalidateCache, type SheetKey } from '../services/supabase';
...
const sheetConfigs: { key: SheetKey; label: string; path: string; cabangField: string; kehadiranField?: string }[] = [
  { key: 'siswa', label: 'Total Siswa', path: '/siswa', cabangField: 'Cabang' },
  { key: 'perkembangan', label: 'Presensi', path: '/perkembangan', cabangField: 'Cabang', kehadiranField: 'Kehadiran' },
  ...
  { key: 'NamaPengajar', label: 'Pengajar', path: '/pengajar', cabangField: 'Cabang' },
];
...
export default function Dashboard() {
  ...
  const loadStat = useCallback(async (index: number, forceRefresh = false) => {
    ...
    try {
    const allData = cfg.key === 'NamaPengajar' ? await fetchPengajarFromKmb() : await fetchAllData(cfg.key, forceRefresh);
      ...
      // Special handling for perkembangan sheets: Presensi breakdown and progress averages
      if (cfg.key === 'perkembangan' && cfg.label === 'Presensi') {
        // count Kehadiran values
        const counts: Record<string, number> = { Hadir: 0, Izin: 0, Sakit: 0, Alpha: 0 };
        filteredData.forEach((row) => {
          const val = String(row[cfg.kehadiranField || 'Kehadiran'] || row['Kehadiran'] || row['kehadiran'] || '').trim();
          if (!val) return;
          const key = val === 'Hadir' ? 'Hadir' : val === 'Izin' ? 'Izin' : val === 'Sakit' ? 'Sakit' : val === 'Alpha' ? 'Alpha' : null;
      ...
      if (cfg.key === 'perkembangan' && cfg.label === 'Perkembangan') {
        // compute averages for Prosen Penguasaan / Penjelasan / Kondisi
        const sums = { penguasaan: 0, penjelasan: 0, kondisi: 0 };
        const counts = { penguasaan: 0, penjelasan: 0, kondisi: 0 };
        const perRowMeans: number[] = [];

        filteredData.forEach((row) => {
          // only consider hadir rows where numeric percents exist
          const hadir = String(row['Kehadiran'] || row['kehadiran'] || '').trim();
          const pPeng = parseFloat(String(row['Prosen Penguasaan'] || row['prosen_penguasaan'] || ''));
          const pPen = parseFloat(String(row['Prosen Penjelasan'] || row['prosen_penjelasan'] || ''));
          const pKon = parseFloat(String(row['Prosen Kondisi'] || row['prosen_kondisi'] || ''));
          const hasPeng = Number.isFinite(pPeng);
          const hasPen = Number.isFinite(pPen);
          const hasKon = Number.isFinite(pKon);
          if (hasPeng) { sums.penguasaan += pPeng; counts.penguasaan += 1; }
          if (hasPen) { sums.penjelasan += pPen; counts.penjelasan += 1; }
          if (hasKon) { sums.kondisi += pKon; counts.kondisi += 1; }
          const rowVals: number[] = [];
          if (hasPeng) rowVals.push(pPeng);
          if (hasPen) rowVals.push(pPen);
          if (hasKon) rowVals.push(pKon);
          if (rowVals.length) {
            const mean = rowVals.reduce((a, b) => a + b, 0) / rowVals.length;
            perRowMeans.push(mean);
          }
        });

        const avgPeng = counts.penguasaan ? sums.penguasaan / counts.penguasaan : 0;
        const avgPen = counts.penjelasan ? sums.penjelasan / counts.penjelasan : 0;
        const avgKon = counts.kondisi ? sums.kondisi / counts.kondisi : 0;
        const overallAvg = perRowMeans.length ? perRowMeans.reduce((a, b) => a + b, 0) / perRowMeans.length : 0;
        ...
        setStats(prev => {
          ...
          next[index] = {
            ...
            totalValue: user && !user.isAdmin ? totalValue : undefined,
            meta: {
              'Rata-rata': Number(overallAvg.toFixed(1)),
              'Prosen Penguasaan': Number(avgPeng.toFixed(1)),
              'Prosen Penjelasan': Number(avgPen.toFixed(1)),
              'Prosen Kondisi': Number(avgKon.toFixed(1)),
            },
            loaded: true,
          };
          return next;
                  ...
                  <>
                    ...
                    {stat.meta && (
                      <div className="mt-2 text-xs text-gray-600 space-y-1">
                        {Object.entries(stat.meta).map(([k, v]) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {user?.isAdmin ? (
          <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg">
            <h3 className="text-lg font-semibold mb-4">🔗 Database Terhubung</h3>
