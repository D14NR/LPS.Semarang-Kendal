...
export default function PelayananJamTambahan() {
  ...
  const loadData = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      console.debug('sample siswaData:', Array.isArray(siswa) ? siswa.slice(0, 3) : siswa);
      console.debug('sample pelayananData:', Array.isArray(pelayanan) ? pelayanan.slice(0, 3) : pelayanan);
    } catch {
  ...
  const cabangOptions = useMemo(() => {
    ...
    siswaData.forEach((row) => {
      const cabang = getRowValue(row, 'Cabang', 'cabang');
      if (cabang) unique.set(cabang.toLowerCase(), cabang);
    });
  ...
  const pengajarOptions = useMemo(() => {
    ...
    const map = new Map<string, string>();
    const wanted = mataPelajaran.trim().toLowerCase();
    pengajarData.forEach((row) => {
      const raw = getRowValue(
        row,
        'bidang_studi',
      ...
      if (!raw) return;
      const parts = raw.split(/[;,|\\/]+/).map((p) => p.trim()).filter(Boolean);
      const matches = parts.some((p) => p.toLowerCase() === wanted);
      if (!matches) return;
      const nama = getRowValue(row, 'nama', 'Pengajar', 'Nama Pengajar', 'nama_pengajar').trim();
  ...
  const jenjangOptions = useMemo(() => {
    const unique = new Map<string, string>();
    siswaData.forEach((row) => {
      const jenjangVal = getRowValue(row, 'Jenjang Studi', 'jenjang_studi', 'Jenjang', 'jenjang');
      if (jenjangVal) unique.set(jenjangVal.toLowerCase(), jenjangVal);
    });
    return Array.from(unique.values()).sort((a, b) => a.localeCompare(b, 'id'));
  }, [siswaData]);

  const effectiveCabang = user?.isAdmin ? selectedCabang : user?.cabang || '';
  const effectiveCabangNorm = (effectiveCabang || '').trim().toLowerCase();
  ...
  const filteredSiswa = useMemo(() => {
    ...
    const wantJenjang = jenjang.trim().toLowerCase();
    return siswaData.filter((row) => {
      const rowJenjang = getRowValue(row, 'Jenjang Studi', 'jenjang_studi', 'Jenjang', 'jenjang').trim().toLowerCase();
      if (rowJenjang !== wantJenjang) return false;
      if (effectiveCabangNorm) {
        const rowCabang = getRowValue(row, 'Cabang', 'cabang').trim().toLowerCase();
        if (rowCabang !== effectiveCabangNorm) return false;
      }
  ...
  }, [siswaData, jenjang, effectiveCabangNorm, searchTerm]);

  useEffect(() => {
    setCurrentPage(1);
  }, [jenjang, searchTerm, effectiveCabangNorm]);
  ...
  useEffect(() => {
    ...
    const normalizedCabang = effectiveCabangNorm;
    const wantJenjang = jenjang.trim().toLowerCase();

    const allowedNis = new Set<string>();
    siswaData.forEach((row) => {
      const rowJenjang = getRowValue(row, 'Jenjang Studi', 'jenjang_studi', 'Jenjang', 'jenjang').trim().toLowerCase();
      if (rowJenjang !== wantJenjang) return;
      if (normalizedCabang) {
        const rowCabang = getRowValue(row, 'Cabang', 'cabang').trim().toLowerCase();
        if (rowCabang !== normalizedCabang) return;
      }
      const nis = getNisValue(row).trim();
    ...
    pelayananData.forEach((row) => {
      ...
      const nis = getRowValue(row, 'Nis', 'nis', 'NIS').trim();
      if (!nis) return;
  ...
  }, [tanggal, mataPelajaran, pengajar, jenjang, effectiveCabangNorm, pelayananData, siswaData]);
  ...
  const handleSubmitPelayanan = async () => {
    ...
    const records = siswaData
      .filter((row) => selectedSiswa[getNisValue(row)])
      .map((row) => ({
        Nis: getNisValue(row) || '',
        'Nama Siswa': getNamaValue(row) || '',
        Tanggal: tanggal,
        'Mata Pelajaran': mataPelajaran,
        Materi: materi,
        Durasi: durasi,
        Pengajar: pengajar,
        Cabang: effectiveCabang || getRowValue(row, 'Cabang', 'cabang') || '',
      }));
    ...
    pelayananData.forEach((row) => {
      const key = [
        getRowValue(row, 'Nis', 'nis', 'NIS'),
        getRowValue(row, 'Tanggal', 'tanggal'),
        getRowValue(row, 'Mata Pelajaran', 'mata_pelajaran', 'mata pelajaran'),
        getRowValue(row, 'Pengajar', 'pengajar', 'nama_pengajar'),
        getRowValue(row, 'Cabang', 'cabang'),
  ...
  const handleTemplateDownload = () => {
    const blob = generateTemplateWorkbook(getTemplateFields(!!user?.isAdmin), 'Template Pelayanan');
  ...
  const handleImportSubmit = async () => {
    ...
    const normalizeKey = (record: Record<string, string>) =>
      [
        getRowValue(record, 'Nis', 'nis', 'NIS'),
        getRowValue(record, 'Tanggal', 'tanggal'),
        getRowValue(record, 'Mata Pelajaran', 'mata_pelajaran', 'mata pelajaran'),
        getRowValue(record, 'Pengajar', 'pengajar', 'nama_pengajar'),
        getRowValue(record, 'Cabang', 'cabang'),
      ]
    ...
    const result = await createBulkRecords('pelayanan', toCreate);
    if (result.success) {
  ...
  const openEditModal = (row: Record<string, string>) => {
    ...
    setFormData({
      Nis: row['Nis'] || '',
      nama: getRowValue(row, 'Nama', 'nama', 'Nama Siswa', 'nama_siswa') || '',
      Tanggal: (() => {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    const result = await updateRecord('pelayanan', identifier, formData);
    if (result.success) {
  ...
  const handleDelete = async () => {
    ...
    setSubmitting(true);
    const result = await deleteRecord('pelayanan', identifier);
    if (result.success) {
  ...
  const sortedPelayanan = useMemo(() => {
    ...
    if (user && !user.isAdmin) {
      const userCab = (user.cabang || '').trim().toLowerCase();
      rows = rows.filter((row) => getRowValue(row, 'Cabang', 'cabang').trim().toLowerCase() === userCab);
    }
    rows.sort((a, b) => {
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {paginatedSiswa.length === 0 ? (
                      ...
                      paginatedSiswa.map((row) => (
                        <tr key={getNisValue(row) || Math.random().toString(36).slice(2, 9)} className="hover:bg-blue-50/50">
                          <td className="px-4 py-3">
                            <input
                                type="checkbox"
                                checked={!!selectedSiswa[getNisValue(row)]}
                                onChange={() => toggleSiswa(getNisValue(row))}
                              className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                            />
                          </td>
                            <td className="px-4 py-3 text-gray-700 font-medium">{getNisValue(row)}</td>
                            <td className="px-4 py-3 text-gray-700">{getNamaValue(row)}</td>
                          <td className="px-4 py-3 text-gray-500">{getRowValue(row, 'Jenjang Studi', 'jenjang_studi', 'Jenjang', 'jenjang') || '-'}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
